...
class TestInterceptor:
    ...
    def test_GET_OK(self, database, capsys):
        ...
        with mock.patch.object(interceptorhandler,'finish',finish):
            with mock.patch.object(interceptorhandler,'date_time_string',date_time_string):
                with mock.patch.object(interceptorhandler,'version_string',version_string):
                    with mock.patch.object(interceptorhandler,'wbufsize', lambda:1):
                        start = datetime.now()
                        request = MockRequest(b"/sensorlog?hum=70&temp=24&id=%s" % bytes(stationid, 'UTF-8'))
                        ihinstance = interceptorhandler(request, ('127.0.0.1',12345), "testserver.example.org")
                        captured = capsys.readouterr()
                        print(captured.out)
                        assert ihinstance.wfile.getvalue() == b'HTTP/1.0 200 OK\r\nServer: BaseHTTP/x.x Python/x.x.x\r\nDate: DATETIME\r\n\r\n'
                        database.names(
                            stationid, "testroom1"
    ...
    def test_GET_SERVER_ERROR(self, database, capsys):
        ...
        interceptorhandler = InterceptorHandlerFactory.getHandler(None)  # no database reference will force an exception
